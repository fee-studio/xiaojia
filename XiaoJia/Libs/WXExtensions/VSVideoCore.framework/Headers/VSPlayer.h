 ...
 */
- (void)player:(VSPlayer *__nonnull)player didPlay:(CMTime)currentTime loopsCount:(NSInteger)loopsCount;

/**
 Called when the item has been changed on the VSPlayer
 */
- (void)player:(VSPlayer *__nonnull)player didChangeItem:(AVPlayerItem *__nullable)item;

/**
 Called when the item has reached end
 */
- (void)player:(VSPlayer *__nonnull)player didReachEndForItem:(AVPlayerItem *__nonnull)item;

/**
 Called when the item is ready to play
 */
- (void)player:(VSPlayer *__nonnull)player itemReadyToPlay:(AVPlayerItem *__nonnull)item;

/**
 Called when the player has setup the renderer so it can receive the image in the
 proper orientation.
 */
- (void)player:(VSPlayer *__nonnull)player didSetupVSImageView:(VSImageView *__nonnull)VSImageView;
 ...
 */
@interface VSPlayer : AVPlayer

/**
 The delegate that will receive the messages
 */
@property (weak, nonatomic) __nullable id<VSPlayerDelegate> delegate;
 ...
 */
@property (strong, nonatomic) VSImageView *__nullable VSImageView;

/**
 Convenient method to return a new instance of a VSPlayer
 */
+ (VSPlayer *__nonnull)player;

/**
 Ask the VSPlayer to send didPlay messages during the playback
 endSendingPlayMessages must be called, otherwise the VSPlayer will never
 be deallocated
 */
- (void)beginSendingPlayMessages;

/**
 Ask the VSPlayer to stop sending didPlay messages during the playback
 */
